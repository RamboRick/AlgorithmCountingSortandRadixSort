...
public class Sorts {
  ...
  public static int[] countingSort(int[] keys, int whichDigit) {
	...
	if(whichDigit > 7 || whichDigit < 0){  
		return null;
	}
	int reset[] = new int[keys.length];
	double wd1 = Math.pow((double)16, (double)(whichDigit +1));
	double wd2 = Math.pow((double)16, (double)whichDigit);
	int wd3 = (int)wd1;
	int wd4 = (int)wd2;
	for(int i = 0; i <keys.length; i++){
		reset[i] = (keys[i]%wd3)/wd4;	
	}
